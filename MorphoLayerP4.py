...
### scalar MAX-TIMES-PLUS MorphoLayer on P4 (Dilation, Erosion, Opening and Closing)
    ...
    def call(self, x):
        ...
        for i in range(4):
            ...
            for j in range(self.num_filters):
                ...
                res_filters.append(tf.reduce_sum(erosion2d(tf.math.divide(x, self.timesKernel[:,j] + tf.keras.backend.epsilon()), tf.divide(kernel_rot[...,j], self.timesKernel[:,j] + tf.keras.backend.epsilon()), self.strides, self.padding),axis=-1))
    ...
    def call(self, x):
        ...
        for i in range(4):
            ...
            for j in range(self.num_filters):
                ...
                ero = erosion2d(tf.math.divide(x, self.timesKernel[:,j] + tf.keras.backend.epsilon()), tf.divide(kernel_rot[...,j], self.timesKernel[:,j] + tf.keras.backend.epsilon()), strides = (1, 1), padding = self.padding)
        ...
        for j in range(4):
            ...
            for k in range(3):
                ...
                for i in range(self.num_filters):
                    ...
                    res_filters.append(dilation2d(tf.math.multiply(y[:,j + k,...], self.timesKernel[k,:,i]), kernel_rot[..., k, :,i],self.strides, self.padding),)
...
class MaxTimesPlusOpeningP4(tf.keras.layers.Layer):
    ...
    def call(self, x):
        ...
        y = tf.concat([x[:,-1:, ...], x, x[:,0:1, ...]], axis = 1)
        y = tf.extract_volume_patches(y, ksizes=(1, 1) + self.kernel_size + (1,), strides = (1, 1, 1, 1, 1), padding=self.padding.upper())
